...
server <- function(input, output, session) {
  ...
  output$plot_sinput_dropdown <- renderPlotly({
    ...
    if (all(is.na(antwortanalyse_data_dropdown()$var_value))) {
      ...
      plot_ly(antwortanalyse_data_dropdown(), 
              x = ~master_id, y = ~percent, color = ~points_individual, 
              colors = ~colour ,  
      ...
      for( i in 1:length(unique_master_id)){
        ...
        plotlylist[[i]] = antwortanalyse_data_dropdown %>% 
          dplyr::filter(master_id == unique_master_id[i]) %>%
          plot_ly(x = ~var_value, y = ~percent, color = ~points_individual, 
                  colors = ~colour,
                  text = ~paste(#"Feldinhalt :", ~feldinhalt_trimmed,
                    "<br> Anzahl :", n_i," von ",N,
                    "<br> Proportion :", round(percent,2)),
          ...
          hide_colorbar() %>% 
          add_annotations(text = unique_master_id[i] ,x = x_midpoint, y = 107, 
                          showarrow = FALSE)
      }
  ...
  output$plot_sinput_fillin <- renderPlotly({
    ...
    for( i in 1:length(unique_master_id)){
      ...
      plotlylist[[i]] = antwortanalyse_data_fillin() %>% 
        dplyr::filter(master_id == unique_master_id[i]) %>%
        plot_ly(x = ~feldname, y = ~percent, color = ~points_individual, colors = ~colour , 
                text = ~paste(#"Feldinhalt :", ~feldinhalt_trimmed,
                  "<br> Anzahl :", n_i," von ",N,
                  "<br> Proportion :", round(percent,2)),
                textposition = "none",
        ...
        layout(barmode = "stack",
               ...
               xaxis = list (title = NULL,
                             tickangle = 90)) %>%
        hide_colorbar() %>% 
        add_annotations(text =  unique_master_id[i] ,x =x_midpoint,y=107, showarrow = FALSE)
    }
