...
server <- function(input, output, session) {
  ...
  greenGradient <- colorRampPalette(c("white", "green"))
  redGradient <- colorRampPalette(c("white", "red"))
  ...
  output$table_output_sinput <- renderGT({
    req(default_table_sinput())
    
    # Extract the numeric values from the "True" and "False" columns
    true_values <- as.numeric(gsub("%", "", default_table_sinput()$True))
    false_values <- as.numeric(gsub("%", "", default_table_sinput()$False))
    
    # Create a gt table
    gt(default_table_sinput()) %>%
      data_color(
...
# Create color gradients
greenGradient <- colorRampPalette(c("white", "green"))(10)
redGradient <- colorRampPalette(c("white", "red"))(10)
...
output$table_output_sinput <- renderDT({
  ...
  datatable(default_table_sinput(), extensions = 'Buttons', options = list(
    ...
    formatStyle('True',
                backgroundColor = styleInterval(seq(0, 90, by = 10), greenGradient)) %>%
    formatStyle('False',
                backgroundColor = styleInterval(seq(0, 90, by = 10), redGradient))
}, class = "display")
